/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package com.mycompany.dao;

import com.mycompany.entities.ToyCategory;
import java.util.List;
import javax.persistence.Query;
import org.hibernate.Session;
import org.hibernate.SessionFactory;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Repository;

/**
 *
 * @author andpa
 */
@Repository
public class ToyCategoryDaoImpl implements ToyCategoryDao{

     @Autowired
    private SessionFactory  sessionfactory;
    
    private Session getSession(){
    return sessionfactory.getCurrentSession();
    }
    @Override
    public List<ToyCategory> findAll() {
          Query q = getSession().createQuery("Select tc From ToyCategory tc");
        List<ToyCategory> list = q.getResultList();
        return list;
    }

    @Override
    public void createOrUpdate(ToyCategory tc) {
         getSession().saveOrUpdate(tc);
    }

    @Override
    public void delete(int id) {
        
    }

    @Override
    public ToyCategory findById(Integer id) {
       return (ToyCategory)getSession().get(ToyCategory.class, id);
    }

    @Override
    public List<ToyCategory> findToyCategorybyTitle(String searchName) {
         throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    }
    
}
